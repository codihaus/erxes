...
import { getAccessTokenByOauthCode, getAPI, setZaloConfigs, ZaloAccessToken, zaloGet, ZaloRefreshToken } from "../../zalo";
...
export const connectAccount = async (req, res) => {
    ...
    const token = await getAccessTokenByOauthCode(config.code, conf.app_id, conf.secret_key)
    debug.error(`connect ${token}`)
    const OAInfo = await zaloGet('getoa', { models, oa_id: config.oa_id, access_token: token?.access_token })
...
export const createOrUpdateAccount = async (Accounts: IAccountModel, accountData: any) => {

    const {oa_id, name, avatar, isNewRefreshToken, access_token, refresh_token} = accountData

    const access_token_expires_in = new Date().getTime() + 86400000
    const refresh_token_expires_in = isNewRefreshToken ? new Date().getTime() + 719 * 3600000 : 0
    ...
    if( refresh_token_expires_in ) data.refresh_token_expires_in = refresh_token_expires_in
