...
export const getAPI = () => {
    if (!Zalo) Zalo = createAPI('https://openapi.zalo.me/v2.0/oa/', {
        headers: {
            "Content-Type": "application/json",
...
export const getAuthAPI = () => {
    if (!ZaloAuth) ZaloAuth = createAPI('https://oauth.zaloapp.com/v4/oa/', {
        headers: {
...
export const setZaloConfigs = (app_id: string = '', secret_key: string = '') => {
    ZaloAppID = app_id
    ZaloSecretKey = secret_key
}

export const getAccessTokenByOauthCode = async (code: string, app_id: string, secret_key: string, callback: Function = () => {}) => {
...
export const zaloGetAccessToken = async (models, oa_id: string = '') => {
    ...
    if( account?.access_token && account?.access_token_expires_in > new Date().getTime() ) {
    ...
    if( account?.refresh_token ){
        ...
        if( token?.access_token ) await createOrUpdateAccount(models.Accounts, { oa_id, access_token, refresh_token: token?.refresh_token })
        return access_token
    }
}
...
export const getRequestConfigs = (config, access_token) => {
    ...
    if( config?.headers ) output.headers = { ...output.headers, ...config.headers }

    return output
}

export const zaloGet = async (path: string = '', config?: ZaloRequestConfig) => {
    const access_token = config?.access_token || await zaloGetAccessToken(config?.models, config?.oa_id)
    return await axios.get(`${OAAPIUrl}${path}`, getRequestConfigs(config, access_token)).then((res: any) => res.data).catch((e: any) => e)
}

export const zaloSend = async (path: string = '', data?: any, config?: ZaloRequestConfig) => {
    const access_token = await zaloGetAccessToken(config?.models, config?.oa_id)
    return await axios.post(`${OAAPIUrl}${path}`, data, getRequestConfigs(config, access_token)).then((res: any) => res.data).catch((e: any) => e)
}